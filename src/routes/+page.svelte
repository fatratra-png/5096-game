  ...
  const tileStyle = (value: number): string => {
    ...
    const fontSize =
      value < 100
        ? "clamp(22px, 7vw, 34px)"
        : value < 1000
          ? "clamp(17px, 5.5vw, 26px)"
          : "clamp(13px, 4.5vw, 20px)";
    ...
    {#if status === "won"}
      <div class="overlay won">
        <div class="overlay-box">
          ...
          <p class="overlay-sub">
            t'as atteint <strong>{TARGET_SCORE}</strong>.
          </p>
          <div class="overlay-actions">
            <button class="btn-continue" on:click={() => (keepPlaying = true)}
              >CONTINUER</button
            >
    ...
    {:else if status === "lost"}
      <div class="overlay lost">
        <div class="overlay-box">
          ...
          <button class="btn-restart lost-btn" on:click={restart}
            >R&Eacute;ESSAYER</button
          >
        </div>
      </div>
    {/if}
